...
from layer import DescartesExtension, RandomDE, MaskDE, LocalDE, SLConv, Mode, MaskLayer
...
class ConvModeNN(BaseModel):
    ...
    def forward(self, x):
        ...
        if x.dim() == 3:
            x = x.unsqueeze(1)
        outs = [torch.flatten(self.de_layers[i](x), 1) for i in range(len(self.hparams.order))]
        outs.append(origin)
        de_out = torch.cat(outs, dim=-1)
