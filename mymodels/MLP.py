from layer import Fast_DE_Conv
import torch
...
class XOR_MLP(BaseModel):
    def __init__(self, hparams, loss=nn.CrossEntropyLoss()):
        super(XOR_MLP, self).__init__(hparams=hparams, loss=loss)
        self.loss = loss        
        self.fc = nn.Linear(self.hparams.input_size, self.hparams.dense_nodes)
        self.out_layer = nn.Linear(self.hparams.dense_nodes, self.hparams.num_classes, bias=False)
    ...
    def add_model_specific_args(parent_parser):  # pragma: no cover
        ...
        parser.add_argument('--dense-nodes', default=5, type=int,
...
class MLP(BaseModel):
    def __init__(self, hparams, loss=nn.CrossEntropyLoss()):
        ...
        self.out_layer = nn.Linear(self.hparams.dense_nodes[-1], self.hparams.num_classes)#, bias=False)
